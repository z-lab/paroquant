...
__device__ __forceinline__ static void warp_reduce(T *psum,
                                                   float (*out_smem)[Num * 4]) {
  // kInterleave = 4
  float fpsum[Num];
#pragma unroll
  for (int i = 0; i < Num; ++i) {
...
__global__ void gemv_kernel(const T *inputs, const uint32_t *weight,
                            const T *scales, const T *zeros, T *outputs,
  ...
  // float(*out_smem)[Num * kInterleave] = reinterpret_cast<float(*)[Num *
  // kInterleave]>(shmem);
  ...
  const int act_k_offset =
      threadIdx.x / (kThreadsNumPerTile * kInterleave) * kStride +
  ...
  const T *scale_ptr =
      scales + blk_row_offset + thd_row_offset + group_offset * OC;
  const T *zeros_ptr =
      zeros + blk_row_offset + thd_row_offset + group_offset * OC;
  ...
  for (int kk = threadIdx.x * kElemsPerThread; kk < IC * kInterleave;
       kk += BlockSize * kElemsPerThread) {
// Load qweight, scales and scaled_zeros
#pragma unroll
    for (int idx = 0; idx < NPerBlock; ++idx) {
      // use float4 to load weights, each thread load 32 int4 numbers (1 x
      // float4, 128 bit)
      *((float4 *)(local_qweights)) =
          *((float4 *)(blk_weight_ptr +
                       (idx * kInterleave * IC + kk) / PACK_FACTOR));
...
// Dequantize (apply s/z) and shuffle elements to match the weight packing
// format
#pragma unroll
      for (int i = 0; i < kShuffleContinous; ++i) {
#pragma unroll
        for (int j = 0; j < kShuffleStrided; ++j) {
          T2 w = *reinterpret_cast<T2 *>(half_weight_buffer +
                                         (i + j * kShuffleContinous) *
                                             kShuffleBasicTile);
          if constexpr (std::is_same<T, half>::value) {
            w = __hfma2(w, __half2half2(local_scale[idx]),
                        __half2half2(local_scaled_zeros[idx]));
          } else {
            w = __hfma2(w, __bfloat162bfloat162(local_scale[idx]),
                        __bfloat162bfloat162(local_scaled_zeros[idx]));
          }
          dequantized_weight[((i * kShuffleStrided + j) * kShuffleBasicTile +
                              0) *
                                 NPerBlock +
                             idx] = w.x;
          dequantized_weight[((i * kShuffleStrided + j) * kShuffleBasicTile +
...
#pragma unroll
      for (int idx = 0; idx < kElemsPerThread / 8; ++idx) {
        ...
        *((float4 *)(local_inputs + idx * 8)) =
            *((float4 *)(local_inputs_ptr + idx * 8));
      }
// Perform the MACs
#pragma unroll
...
#pragma unroll
        for (int y = 0; y < kElemsPerThread; ++y) {
          if constexpr (std::is_same<T, half>::value) {
            *reinterpret_cast<half2 *>(psum + batch_idx * NPerBlock + x * 2) =
                __hfma2(*reinterpret_cast<half2 *>(dequantized_weight +
                                                   y * NPerBlock + x * 2),
                        __half2half2(local_inputs[y]),
                        *reinterpret_cast<half2 *>(
                            psum + batch_idx * NPerBlock + x * 2));
          } else {
            *reinterpret_cast<nv_bfloat162 *>(psum + batch_idx * NPerBlock +
                                              x * 2) =
                __hfma2(*reinterpret_cast<nv_bfloat162 *>(
                            dequantized_weight + y * NPerBlock + x * 2),
  ...
  // Num * Interleave = batch * NPerBlock * Interleave -> 1 thread_block write
  // back num
...
Args:
  ...
  blockDim_x: size of thread block, dimension x, where blockDim_x *
workload_per_thread = IC; blockDim_y: size of thread block, dimension y, where
blockDim_y * gridDim_y = OC;

Returns:
  out_feats: tensor of shape [B, OC];
*/
torch::Tensor gemv_forward_cuda_new(torch::Tensor _in_feats,
                                    torch::Tensor _kernel,
                                    torch::Tensor _scaling_factors,
                                    torch::Tensor _zeros, int m, int n, int k,
                                    int group_size) {
  ...
  DISPATCH_PYTORCH_DTYPE_TO_CTYPE_FP16(data_type, ctype, {
    ...
    auto scaling_factors =
        reinterpret_cast<ctype *>(_scaling_factors.data_ptr());
    ...
    if (group_size == 128) {
      ...
      case 6:
        ...
        break;
      case 7:
        gemv_kernel<N_PER_BLOCK, 7, BLOCK_SIZE, 128>
            <<<num_blocks, num_threads, 0, stream>>>(
                in_feats, kernel, scaling_factors, zeros, out_feats, k, n);
        break;
      default:
