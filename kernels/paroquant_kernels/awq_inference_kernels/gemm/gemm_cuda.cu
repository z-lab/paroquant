...
#define KERNEL_LAUNCH_CODE                                                     \
  int num_mn_tiles = (num_in_feats + CTA_M - 1) / CTA_M *                      \
                     (num_out_channels + CTA_N - 1) / CTA_N;                   \
  ...
  constexpr int SCALES_SMEM_SIZE = (G >= CTA_K)                                \
                                       ? (CTA_N / (G / CTA_K) * STAGES * 2)    \
                                       : (CTA_N * (CTA_K / G) * STAGES * 2);   \
  constexpr int kSmemByteSize =                                                \
      (CTA_M * (CTA_K + SMEM_PAD_A) +                                          \
       CTA_N * (CTA_K + SMEM_PAD_B) / kInterleave + SCALES_SMEM_SIZE) *        \
      STAGES * sizeof(ctype);                                                  \
  ...
  auto kernel_func = gemm_w4a16_T1<CTA_M, CTA_N, CTA_K, WARP_M, WARP_N,        \
                                   WARP_K, STAGES, G, SPLITK, ctype>;          \
...
template <typename T>
__inline__ __device__ void ldmatrix_m8n8_x4_b16(T *shared_warp, int ax0_0,
                                                uint32_t addr) {
...
template <typename T>
__inline__ __device__ void ldmatrix_m8n8_x4_trans_b16(T *shared_warp, int ax0_0,
                                                      uint32_t addr) {
...
mma_m16n8k16_f16f16f16(half *C_warp, half *A_shared_warp, half *B_shared_warp) {
  __asm__ __volatile__(
      ...
      : "r"(((unsigned *)A_shared_warp)[0]),
        ...
        "r"(((unsigned *)B_shared_warp)[1]), "r"(((unsigned *)C_warp)[0]),
        "r"(((unsigned *)C_warp)[1]));
}

__device__ __inline__ void
mma_m16n8k16_bf16bf16f32(float *C_warp, nv_bfloat16 *A_shared_warp,
                         nv_bfloat16 *B_shared_warp) {
  ...
  __asm__ __volatile__(
      ...
      : "r"(((unsigned *)A_shared_warp)[0]),
        ...
        "r"(((unsigned *)A_shared_warp)[3]),
        "r"(((unsigned *)B_shared_warp)[0]),
        "r"(((unsigned *)B_shared_warp)[1]), "f"(C_warp[0]), "f"(C_warp[1]),
        "f"(C_warp[2]), "f"(C_warp[3]));
}

template <int CTA_M, int CTA_N, int CTA_K, int CTA_SIZE, int SHARED_K_ITERS,
          int STAGES, typename T>
__device__ __inline__ void
global_to_share_one_stage_A(T *src, T *dst, int global_nrows, int global_ncols,
                            int cta_offset_m, int cta_offset_n,
                            int cta_offset_k, int global_iter_k,
                            int shared_iter_k, bool mask) {
  constexpr int threads_needed = (CTA_M * CTA_K) / PACK_SIZE / SHARED_K_ITERS;
  constexpr int threads_used =
      threads_needed < CTA_SIZE ? threads_needed : CTA_SIZE;
  constexpr int total_global_iters = (CTA_M * CTA_K) / PACK_SIZE / threads_used;
  constexpr int partial_global_iters =
      (total_global_iters + SHARED_K_ITERS - 1) / SHARED_K_ITERS;
  ...
  bool local_mask =
      mask & (threadIdx.y * WARP_SIZE + threadIdx.x < threads_used);
...
#pragma unroll
  for (int _global_iter = 0; _global_iter < partial_global_iters;
    ...
    int ld_row = global_iter * cta_step_m_or_n +
                 threadIdx.y * warp_step_m_or_n +
    ...
    uint4 *src_ptr =
        (uint4
             *)(src + (ld_row + cta_offset_m) * global_ncols +
                ld_col * PACK_SIZE + global_iter_k * CTA_K +
                cta_offset_k); // cta_offset_m * global_ncols + global_iter *
                               // cta_step_m_or_n * global_ncols + threadIdx.y *
                               // warp_step_m_or_n * global_ncols + (threadIdx.x
                               // / threads_per_row) * global_ncols +
                               // global_iter_k * CTA_K + (threadIdx.x %
                               // threads_per_row) * PACK_SIZE);
...
template <int CTA_M, int CTA_N, int CTA_K, int CTA_SIZE, int SHARED_K_ITERS,
          int STAGES, typename T>
__device__ __inline__ void
global_to_share_one_stage_B(T *src, T *dst, int global_ncols, int cta_offset_m,
                            int cta_offset_n, int cta_offset_k,
                            int global_iter_k, int shared_iter_k, bool mask) {
  constexpr int threads_needed =
      (CTA_N / kInterleave * CTA_K) / PACK_SIZE / SHARED_K_ITERS;
  constexpr int threads_used =
      threads_needed < CTA_SIZE ? threads_needed : CTA_SIZE;
  constexpr int total_global_iters =
      (CTA_N / kInterleave * CTA_K) / PACK_SIZE / threads_used;
  constexpr int partial_global_iters =
      (total_global_iters + SHARED_K_ITERS - 1) / SHARED_K_ITERS;
  constexpr int cta_step_m_or_n = (threads_used * PACK_SIZE) / CTA_K;
  constexpr int warp_step_m_or_n = (WARP_SIZE * PACK_SIZE) / CTA_K;
...
#pragma unroll
  for (int _global_iter = 0; _global_iter < partial_global_iters;
    ...
    void *dst_ptr =
        (void *)(dst + (ld_row * kSmemCol + ld_col_swizzled * PACK_SIZE));
    uint4 *src_ptr =
        (uint4 *)(src + global_iter_k * CTA_K +
                  cta_offset_n / kInterleave * global_ncols +
...
template <int CTA_M, int CTA_N, int CTA_K, int CTA_SIZE, int STAGES, int G,
          typename T>
__device__ __inline__ void global_to_share_one_stage_scales(
    T *src, T *dst, T *src_z, T *dst_z, int global_ncols, int cta_offset_m,
    int cta_offset_n, int cta_offset_k, int global_iter_k, int shared_iter_k,
    bool mask) {
  ...
  void *dst_ptr = (void *)(dst + (threadIdx.x / threads_per_row) * kSmemCol +
                           (threadIdx.x % threads_per_row) * PACK_SIZE);
  uint4 *src_ptr = (uint4 *)(src + g_idx * global_ncols + cta_offset_n +
                             (threadIdx.x / threads_per_row) * global_ncols +
                             (threadIdx.x % threads_per_row) * PACK_SIZE);
  void *dst_ptr_z =
      (void *)(dst_z + (threadIdx.x / threads_per_row) * kSmemCol +
               (threadIdx.x % threads_per_row) * PACK_SIZE);
  uint4 *src_ptr_z = (uint4 *)(src_z + g_idx * global_ncols + cta_offset_n +
                               (threadIdx.x / threads_per_row) * global_ncols +
                               (threadIdx.x % threads_per_row) * PACK_SIZE);
  if (STAGES > 1) {
    uint32_t addr = cast_smem_ptr_to_uint(dst_ptr);
...
template <int CTA_M, int CTA_N, int CTA_K, int STAGES, int shared_iters,
          typename T>
__device__ __inline__ void
share_to_reg_one_stage_A(T *src, T *dst, int warp_offset_m, int warp_offset_n,
                         int warp_offset_k, int k_0_1) {
  constexpr int kSmemCol = CTA_K + SMEM_PAD_A;
...
template <int CTA_M, int CTA_N, int CTA_K, int STAGES, bool ldmatrix,
          int shared_iters, int G, typename T>
__device__ __inline__ void
share_to_reg_one_stage_B(T *src, T *src_scales, T *src_zeros, T *dst,
                         T *dst_fp16, int warp_offset_m, int warp_offset_n,
                         int warp_offset_k, int k_0_1) {
...
#pragma unroll
    for (int shared_iter = 0; shared_iter < shared_iters; ++shared_iter) {
      void *addr_ptr =
          (void *)(src + warp_offset_n / kInterleave * kSmemCol +
                   shared_iter * 16 / kInterleave * kSmemCol + k_0_1 * 16 +
                   r * kSmemCol + c_swizzled + warp_offset_k);
      uint32_t addr = cast_smem_ptr_to_uint(addr_ptr);
      ldmatrix_m8n8_x4_b16(dst, shared_iter, addr);
    }
  }
...
#pragma unroll
  for (int shared_iter = 0; shared_iter < shared_iters; ++shared_iter) {
    T scale = src_scales[(warp_offset_k / G) * CTA_N + warp_offset_n +
                         16 * shared_iter + 8 * (k_0_1 % 2) + threadIdx.x / 4];
    T zero = src_zeros[(warp_offset_k / G) * CTA_N + warp_offset_n +
                       16 * shared_iter + 8 * (k_0_1 % 2) + threadIdx.x / 4];
    T2 scale2, zero2;
    ...
    T2 loaded[4];
    dequantize_s4_to_fp16x2<T>(
        *reinterpret_cast<half2 *>(dst + (k_0_1 % 2) * 4 + (k_0_1 / 2 * 2) +
                                   shared_iter * 8),
...
template <int CTA_M, int CTA_N, int CTA_K, int WARP_M, int WARP_N, int WARP_K,
          int STAGES, int G, int SPLITK, typename T>
__global__ void gemm_w4a16_T1(T *__restrict__ A, T *__restrict__ B,
                              T *__restrict__ scales, T *__restrict__ zeros,
                              T *__restrict__ C, int *__restrict__ semaphores,
                              int M, int N, int K) {
  using DTypeAccum = typename std::conditional<std::is_same<T, half>::value,
                                               half, float>::type;
  ...
  const uint2 block_idx_mapping =
      get_block_idx_mapping(blockIdx_m, blockIdx_n, log_tile);
  ...
  T A_shared_warp_[2][WARP_M * INTRIN_K / WARP_SIZE];
  T B_shared_warp_[2][WARP_N * 32 / WARP_SIZE];
  T B_shared_warp_tmp_[2][WARP_N * 16 / WARP_SIZE];
...
#pragma unroll
  for (k_0_0_ld = 0; k_0_0_ld < prologue_stages; ++k_0_0_ld) {
    global_to_share_one_stage_A<CTA_M, CTA_N, CTA_K, CTA_SIZE, 1, STAGES>(
        A, A_shared + k_0_0_ld * kSmemSizeAPerStage, M, K, cta_offset_m,
        cta_offset_n, cta_offset_k, k_0_0_ld, 0, true);
    global_to_share_one_stage_B<CTA_M, CTA_N, CTA_K, CTA_SIZE, 1, STAGES>(
        B, B_shared + k_0_0_ld * kSmemSizeBPerStage, K, cta_offset_m,
        cta_offset_n, cta_offset_k, k_0_0_ld, 0, true);
    global_to_share_one_stage_scales<CTA_M, CTA_N, CTA_K, CTA_SIZE, STAGES, G>(
        scales,
        scales_shared +
            (k_0_0_ld / scales_load_interval * scales_per_load) * CTA_N,
        zeros,
        zeros_shared +
            (k_0_0_ld / scales_load_interval * scales_per_load) * CTA_N,
        N, cta_offset_m, cta_offset_n, cta_offset_k, k_0_0_ld, 0,
  ...
  share_to_reg_one_stage_A<CTA_M, CTA_N, CTA_K, STAGES, WARP_M / INTRIN_M>(
      A_shared, A_shared_warp_[0], warp_offset_m, warp_offset_n, warp_offset_k,
      0);
  share_to_reg_one_stage_B<CTA_M, CTA_N, CTA_K, STAGES, true, WARP_N / INTRIN_N,
                           G>(B_shared, scales_shared, zeros_shared,
                              B_shared_warp_tmp_[0], B_shared_warp_[0],
                              warp_offset_m, warp_offset_n, warp_offset_k, 0);
...
#pragma unroll
    for (int iter_k = 0; iter_k < SHARED_K_ITERS; ++iter_k) {
      A_shared_this_compute_stage =
          A_shared + compute_stage * kSmemSizeAPerStage;
      B_shared_this_compute_stage =
          B_shared + compute_stage * kSmemSizeBPerStage;
      scales_shared_this_compute_stage =
          scales_shared +
          (compute_stage / scales_load_interval * scales_per_load) * CTA_N;
      zeros_shared_this_compute_stage =
          zeros_shared +
          (compute_stage / scales_load_interval * scales_per_load) * CTA_N;
      share_to_reg_one_stage_A<CTA_M, CTA_N, CTA_K, STAGES, WARP_M / INTRIN_M>(
          A_shared_this_compute_stage, A_shared_warp_[(iter_k + 1) % 2],
          warp_offset_m, warp_offset_n, warp_offset_k,
          (iter_k + 1) % SHARED_K_ITERS);
      if ((iter_k + 1) % kInterleave == 0) {
        if (compute_stage % 2 == 1) {
          share_to_reg_one_stage_B<CTA_M, CTA_N, CTA_K, STAGES, true,
                                   WARP_N / INTRIN_N, G>(
              B_shared_this_compute_stage, scales_shared_this_compute_stage,
              zeros_shared_this_compute_stage, B_shared_warp_tmp_[1],
              B_shared_warp_[((iter_k + 1) / 2) % 2], warp_offset_m,
              warp_offset_n, warp_offset_k, (iter_k + 1) % SHARED_K_ITERS);
        } else {
          share_to_reg_one_stage_B<CTA_M, CTA_N, CTA_K, STAGES, true,
                                   WARP_N / INTRIN_N, G>(
              B_shared_this_compute_stage, scales_shared_this_compute_stage,
              zeros_shared_this_compute_stage, B_shared_warp_tmp_[0],
              B_shared_warp_[((iter_k + 1) / 2) % 2], warp_offset_m,
              warp_offset_n, warp_offset_k, (iter_k + 1) % SHARED_K_ITERS);
        }
      } else {
        if (compute_stage % 2 == 1) {
          share_to_reg_one_stage_B<CTA_M, CTA_N, CTA_K, STAGES, false,
                                   WARP_N / INTRIN_N, G>(
              B_shared_this_compute_stage, scales_shared_this_compute_stage,
              zeros_shared_this_compute_stage, B_shared_warp_tmp_[1],
              B_shared_warp_[((iter_k + 1) / 2) % 2], warp_offset_m,
        ...
        for (int j_0_4 = 0; j_0_4 < WARP_N / INTRIN_N; ++j_0_4) {
          if constexpr (std::is_same<T, half>::value) {
            mma_m16n8k16_f16f16f16(
                C_warp + i_0_3 * WARP_N / INTRIN_N * 8 + j_0_4 * 8,
      ...
      if (iter_k < WARP_K / INTRIN_K - 1) {
        ...
        global_to_share_one_stage_A<CTA_M, CTA_N, CTA_K, CTA_SIZE,
            ...
            cta_offset_n, cta_offset_k, k_0_0_ld, iter_k,
            k_0_0_ld < gemm_iters);
        global_to_share_one_stage_B<CTA_M, CTA_N, CTA_K, CTA_SIZE,
                                    WARP_K / INTRIN_K, STAGES>(
            B, B_shared + ld_stage * kSmemSizeBPerStage, K, cta_offset_m,
            cta_offset_n, cta_offset_k, k_0_0_ld, iter_k,
            k_0_0_ld < gemm_iters);
      }
      ...
      if (iter_k == WARP_K / INTRIN_K - 2) {
        ...
        global_to_share_one_stage_A<CTA_M, CTA_N, CTA_K, CTA_SIZE,
                                    WARP_K / INTRIN_K, STAGES>(
            A, A_shared + ld_stage * kSmemSizeAPerStage, M, K, cta_offset_m,
            cta_offset_n, cta_offset_k, k_0_0_ld, iter_k + 1,
            k_0_0_ld < gemm_iters);
        global_to_share_one_stage_B<CTA_M, CTA_N, CTA_K, CTA_SIZE,
                                    WARP_K / INTRIN_K, STAGES>(
            B, B_shared + ld_stage * kSmemSizeBPerStage, K, cta_offset_m,
            cta_offset_n, cta_offset_k, k_0_0_ld, iter_k + 1,
            k_0_0_ld < gemm_iters);
        global_to_share_one_stage_scales<CTA_M, CTA_N, CTA_K, CTA_SIZE, STAGES,
                                         G>(
            scales,
            scales_shared +
                (ld_stage / scales_load_interval * scales_per_load) * CTA_N,
            zeros,
            zeros_shared +
                (ld_stage / scales_load_interval * scales_per_load) * CTA_N,
            N, cta_offset_m, cta_offset_n, cta_offset_k, k_0_0_ld, iter_k,
            k_0_0_ld < gemm_iters && k_0_0_ld % scales_load_interval == 0);
        if constexpr (STAGES > 1) {
          __pipeline_commit();
          __pipeline_wait_prior(STAGES - 2);
...
#pragma unroll
              for (int local_id = 0; local_id < OP_M * 16 / WARP_SIZE;
                   ++local_id) {
                if (z > 0) {
                  C_warp[ax0_0_1 * WARP_N / INTRIN_N * 8 + ax1_0_1 * 8 +
                         local_id] +=
                      C_shared[warp_offset_m * CTA_N + ax0_0_1 * OP_M * CTA_N +
                               warp_offset_n + ax1_0_1 * 16 +
                ...
                C_shared[warp_offset_m * CTA_N + ax0_0_1 * OP_M * CTA_N +
                         warp_offset_n + ax1_0_1 * 16 +
                         ((local_id % 4) / 2 * 8 + (threadIdx.x / 4)) * CTA_N +
                         (local_id / 4) * 8 + (local_id % 2) +
                         (threadIdx.x % 4) * 2] =
                    C_warp[ax0_0_1 * WARP_N / INTRIN_N * 8 + ax1_0_1 * 8 +
                           local_id];
            ...
            for (int local_id = 0; local_id < OP_M * 16 / WARP_SIZE;
              ...
              if (write_row < M) {
                half2 *existing_psum_ptr = reinterpret_cast<half2 *>(
                    C + write_row * N + cta_offset_n + warp_offset_n +
                ...
                *existing_psum_ptr =
                    __hadd2(*existing_psum_ptr,
                            *reinterpret_cast<half2 *>(
                                C_warp + ax0_0_1 * WARP_N / INTRIN_N * 8 +
                                ax1_0_1 * 8 + local_id));
              }
            };
          }
          ...
          for (int ax1_0_1 = 0; ax1_0_1 < WARP_N / INTRIN_N; ++ax1_0_1) {
            for (int local_id = 0; local_id < OP_M * 16 / WARP_SIZE;
              ...
              int write_row = cta_offset_m + warp_offset_m + ax0_0_1 * OP_M +
                              ((local_id % 4) / 2 * 8 + (threadIdx.x / 4));
              if (write_row < M) {
                *reinterpret_cast<half2 *>(C + write_row * N + cta_offset_n +
                                           warp_offset_n + ax1_0_1 * 16 +
                                           (local_id / 4) * 8 + (local_id % 2) +
                                           (threadIdx.x % 4) * 2) =
                    *reinterpret_cast<half2 *>(C_warp +
                                               ax0_0_1 * WARP_N / INTRIN_N * 8 +
                                               ax1_0_1 * 8 + local_id);
              }
            };
          }
...
#pragma unroll
    for (int i = 0; i < CTA_M * CTA_N / CTA_SIZE_MN / 2; ++i) {
      ((nv_bfloat162 *)C_warp16)[i] =
          __float22bfloat162_rn(((float2 *)C_warp)[i]);
    }

    // the following is the same as fp16. Maybe there is a neat way to implement
    // this.
    if constexpr (SLICES > 1) {
#pragma unroll
      for (int z = 0; z < SLICES; ++z) {
...
#pragma unroll
              for (int local_id = 0; local_id < OP_M * 16 / WARP_SIZE;
                   ++local_id) {
                if (z > 0) {
                  C_warp16[ax0_0_1 * WARP_N / INTRIN_N * 8 + ax1_0_1 * 8 +
                           local_id] +=
                      C_shared[warp_offset_m * CTA_N + ax0_0_1 * OP_M * CTA_N +
                               warp_offset_n + ax1_0_1 * 16 +
                               ((local_id % 4) / 2 * 8 + (threadIdx.x / 4)) *
        ...
        for (int ax0_0_1 = 0; ax0_0_1 < WARP_M / INTRIN_M; ++ax0_0_1) {
          for (int ax1_0_1 = 0; ax1_0_1 < WARP_N / INTRIN_N; ++ax1_0_1) {
            for (int local_id = 0; local_id < OP_M * 16 / WARP_SIZE;
              ...
              if (write_row < M) {
                nv_bfloat162 *existing_psum_ptr =
                ...
                *existing_psum_ptr =
                    __hadd2(*existing_psum_ptr,
                            *reinterpret_cast<nv_bfloat162 *>(
                                C_warp16 + ax0_0_1 * WARP_N / INTRIN_N * 8 +
                                ax1_0_1 * 8 + local_id));
              }
            };
          }
...
template <int CTA_M, int CTA_N, int CTA_K, int CTA_SIZE, int SHARED_K_ITERS,
          int STAGES, typename T>
__device__ __inline__ void global_to_share_one_stage_A_T2(
    T *src, T *dst, int global_nrows, int global_ncols, int cta_offset_m,
...
#pragma unroll
  for (int _global_iter = 0; _global_iter < partial_global_iters;
    ...
    void *dst_ptr = (void *)(dst + ld_row * kSmemCol + ld_col_swizzled);
    uint4 *src_ptr =
        (uint4 *)(src + (ld_row + cta_offset_m) * global_ncols +
                  ld_col * PACK_SIZE +
                  global_iter_k *
                      CTA_K); // cta_offset_m * global_ncols + global_iter *
                              // cta_step_m_or_n * global_ncols + threadIdx.y *
                              // warp_step_m_or_n * global_ncols + (threadIdx.x
                              // / threads_per_row) * global_ncols +
                              // global_iter_k * CTA_K + (threadIdx.x %
...
template <int CTA_M, int CTA_N, int CTA_K, int CTA_SIZE, int SHARED_K_ITERS,
          int STAGES, typename T>
__device__ __inline__ void global_to_share_one_stage_B_T2(
    T *src, T *dst, int global_ncols, int cta_offset_m, int cta_offset_n,
    int global_iter_k, int shared_iter_k, bool mask) {
  constexpr int threads_needed =
      (CTA_N / kInterleave * CTA_K) / PACK_SIZE / SHARED_K_ITERS;
  constexpr int threads_used =
...
#pragma unroll
  for (int _global_iter = 0; _global_iter < partial_global_iters;
    ...
    void *dst_ptr =
        (void *)(dst + (ld_row * kSmemCol + ld_col_swizzled * PACK_SIZE));
    uint4 *src_ptr = (uint4 *)(src + global_iter_k * CTA_K +
                               cta_offset_n / kInterleave * global_ncols +
...
template <int CTA_M, int CTA_N, int CTA_K, int CTA_SIZE, int STAGES, int G,
          typename T>
__device__ __inline__ void global_to_share_one_stage_scales_T2(
    T *src, T *dst, T *src_z, T *dst_z, int global_ncols, int cta_offset_m,
    int cta_offset_n, int global_iter_k, int shared_iter_k, bool mask) {
  ...
  uint4 *src_ptr = (uint4 *)(src + g_idx * global_ncols + cta_offset_n +
                             (threadIdx.x % threads_per_row) * PACK_SIZE);
  void *dst_ptr_z =
      (void *)(dst_z + (threadIdx.x % threads_per_row) * PACK_SIZE);
  uint4 *src_ptr_z = (uint4 *)(src_z + g_idx * global_ncols + cta_offset_n +
                               (threadIdx.x % threads_per_row) * PACK_SIZE);
  if (STAGES > 1) {
    uint32_t addr = cast_smem_ptr_to_uint(dst_ptr);
...
template <int CTA_M, int CTA_N, int CTA_K, int STAGES, int shared_iters,
          typename T>
__device__ __inline__ void
share_to_reg_one_stage_A_T2(T *src, T *dst, int warp_offset_m,
...
template <int CTA_M, int CTA_N, int CTA_K, int STAGES, bool ldmatrix,
          int shared_iters, int G, typename T>
__device__ __inline__ void
share_to_reg_one_stage_B_T2(T *src, T *src_scales, T *src_zeros, T *dst,
                            T *dst_fp16, int warp_offset_m, int warp_offset_n,
                            int k_0_1) {
  using T2 = typename std::conditional<std::is_same<T, half>::value, half2,
                                       nv_bfloat162>::type;
...
#pragma unroll
    for (int shared_iter = 0; shared_iter < shared_iters; ++shared_iter) {
      void *addr_ptr = (void *)(src + warp_offset_n / kInterleave * kSmemCol +
                                shared_iter * 16 / kInterleave * kSmemCol +
...
#pragma unroll
  for (int shared_iter = 0; shared_iter < shared_iters; ++shared_iter) {
    T scale = src_scales[warp_offset_n + 16 * shared_iter + 8 * (k_0_1 % 2) +
                         threadIdx.x / 4];
    T zero = src_zeros[warp_offset_n + 16 * shared_iter + 8 * (k_0_1 % 2) +
                       threadIdx.x / 4];
    T2 scale2, zero2;
    if constexpr (std::is_same<T, half>::value) {
      scale2 = __half2half2(scale);
...
template <int CTA_M, int CTA_N, int CTA_K, int WARP_M, int WARP_N, int WARP_K,
          int STAGES, int G, typename T>
__global__ void gemm_w4a16_T2(T *__restrict__ A, T *__restrict__ B,
                              T *__restrict__ scales, T *__restrict__ zeros,
...
#pragma unroll
  for (k_0_0_ld = 0; k_0_0_ld < prologue_stages; ++k_0_0_ld) {
    global_to_share_one_stage_A_T2<CTA_M, CTA_N, CTA_K, CTA_SIZE, 1, STAGES>(
        A, A_shared + k_0_0_ld * kSmemSizeAPerStage, M, K, cta_offset_m,
        cta_offset_n, k_0_0_ld, 0, true);
    global_to_share_one_stage_B_T2<CTA_M, CTA_N, CTA_K, CTA_SIZE, 1, STAGES>(
        B, B_shared + k_0_0_ld * kSmemSizeBPerStage, K, cta_offset_m,
        cta_offset_n, k_0_0_ld, 0, true);
    global_to_share_one_stage_scales_T2<CTA_M, CTA_N, CTA_K, CTA_SIZE, STAGES,
                                        G>(
        scales, scales_shared + (k_0_0_ld / scales_load_interval) * CTA_N,
        zeros, zeros_shared + (k_0_0_ld / scales_load_interval) * CTA_N, N,
        cta_offset_m, cta_offset_n, k_0_0_ld, 0,
        k_0_0_ld < gemm_iters && k_0_0_ld % scales_load_interval == 0);
    if constexpr (STAGES > 1)
      __pipeline_commit();
  }
  ...
  share_to_reg_one_stage_B_T2<CTA_M, CTA_N, CTA_K, STAGES, true,
                              WARP_N / INTRIN_N, G>(
      B_shared, scales_shared, zeros_shared, B_shared_warp_tmp_[0],
      B_shared_warp_[0], warp_offset_m, warp_offset_n, 0);
  constexpr int SHARED_K_ITERS = WARP_K / INTRIN_K;
  ...
  for (; k_0_0 < gemm_iters; ++k_0_0, ++k_0_0_ld) {
    ...
    for (int iter_k = 0; iter_k < SHARED_K_ITERS; ++iter_k) {
      ...
      B_shared_this_compute_stage =
          B_shared + compute_stage * kSmemSizeBPerStage;
      scales_shared_this_compute_stage =
          scales_shared + (compute_stage / scales_load_interval) * CTA_N;
      zeros_shared_this_compute_stage =
          zeros_shared + (compute_stage / scales_load_interval) * CTA_N;
      share_to_reg_one_stage_A_T2<CTA_M, CTA_N, CTA_K, STAGES,
                                  WARP_M / INTRIN_M>(
          A_shared_this_compute_stage, A_shared_warp_[(iter_k + 1) % 2],
      ...
      if ((iter_k + 1) % kInterleave == 0) {
        if (compute_stage % 2 == 1) {
          share_to_reg_one_stage_B_T2<CTA_M, CTA_N, CTA_K, STAGES, true,
              ...
              zeros_shared_this_compute_stage, B_shared_warp_tmp_[1],
              B_shared_warp_[((iter_k + 1) / 2) % 2], warp_offset_m,
              warp_offset_n, (iter_k + 1) % SHARED_K_ITERS);
        } else {
          share_to_reg_one_stage_B_T2<CTA_M, CTA_N, CTA_K, STAGES, true,
                                      WARP_N / INTRIN_N, G>(
              B_shared_this_compute_stage, scales_shared_this_compute_stage,
        ...
        if (compute_stage % 2 == 1) {
          share_to_reg_one_stage_B_T2<CTA_M, CTA_N, CTA_K, STAGES, false,
                                      WARP_N / INTRIN_N, G>(
              B_shared_this_compute_stage, scales_shared_this_compute_stage,
              zeros_shared_this_compute_stage, B_shared_warp_tmp_[1],
              B_shared_warp_[((iter_k + 1) / 2) % 2], warp_offset_m,
      ...
      if (iter_k < WARP_K / INTRIN_K - 1) {
        if constexpr (STAGES == 1)
          __syncthreads();
        global_to_share_one_stage_A_T2<CTA_M, CTA_N, CTA_K, CTA_SIZE,
                                       WARP_K / INTRIN_K, STAGES>(
            A, A_shared + ld_stage * kSmemSizeAPerStage, M, K, cta_offset_m,
            cta_offset_n, k_0_0_ld, iter_k, k_0_0_ld < gemm_iters);
        global_to_share_one_stage_B_T2<CTA_M, CTA_N, CTA_K, CTA_SIZE,
                                       WARP_K / INTRIN_K, STAGES>(
            B, B_shared + ld_stage * kSmemSizeBPerStage, K, cta_offset_m,
            cta_offset_n, k_0_0_ld, iter_k, k_0_0_ld < gemm_iters);
      }
      ...
      if (iter_k == WARP_K / INTRIN_K - 2) {
        ...
        global_to_share_one_stage_A_T2<CTA_M, CTA_N, CTA_K, CTA_SIZE,
                                       WARP_K / INTRIN_K, STAGES>(
            A, A_shared + ld_stage * kSmemSizeAPerStage, M, K, cta_offset_m,
            cta_offset_n, k_0_0_ld, iter_k + 1, k_0_0_ld < gemm_iters);
        global_to_share_one_stage_B_T2<CTA_M, CTA_N, CTA_K, CTA_SIZE,
                                       WARP_K / INTRIN_K, STAGES>(
            B, B_shared + ld_stage * kSmemSizeBPerStage, K, cta_offset_m,
            cta_offset_n, k_0_0_ld, iter_k + 1, k_0_0_ld < gemm_iters);
        global_to_share_one_stage_scales_T2<CTA_M, CTA_N, CTA_K, CTA_SIZE,
                                            STAGES, G>(
            scales, scales_shared + (ld_stage / scales_load_interval) * CTA_N,
            zeros, zeros_shared + (ld_stage / scales_load_interval) * CTA_N, N,
        ...
        if (write_row < M) {
          if constexpr (std::is_same<T, half>::value) {
            ...
            *reinterpret_cast<nv_bfloat162 *>(
                ...
                ax1_0_1 * 16 + (local_id / 4) * 8 + (local_id % 2) +
                (threadIdx.x % 4) * 2) =
                (__float22bfloat162_rn(*reinterpret_cast<float2 *>(
                    C_warp + ax0_0_1 * WARP_N / INTRIN_N * 8 + ax1_0_1 * 8 +
                    local_id)));
          }
        }
...
torch::Tensor gemm_forward_cuda_new(torch::Tensor _in_feats,
  ...
  DISPATCH_PYTORCH_DTYPE_TO_CTYPE_FP16(data_type, ctype, {
    ...
    if (num_out_feats <= 32) {
      ...
      constexpr int kSmemByteSize =
          (CTA_M * (CTA_K + SMEM_PAD_A) +
           CTA_N * (CTA_K + SMEM_PAD_B) / kInterleave + CTA_N) *
          STAGES * sizeof(ctype);
      if (kSmemByteSize >= 99 * 1024) {
        printf("This kernel requires %d Bytes of shared memory, which exceeds "
               "device limit.\n",
      ...
      dim3 threads_per_block(WARP_SIZE, NUM_WARPS);
      auto kernel_func = gemm_w4a16_T2<CTA_M, CTA_N, CTA_K, WARP_M, WARP_N,
                                       WARP_K, STAGES, G, ctype>;
      cudaFuncSetAttribute(kernel_func,
                           cudaFuncAttributeMaxDynamicSharedMemorySize,
                           kSmemByteSize);
      kernel_func<<<num_blocks, threads_per_block, kSmemByteSize>>>(
