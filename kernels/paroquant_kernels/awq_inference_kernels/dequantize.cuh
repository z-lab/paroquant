...
__inline__ __device__ void dequantize_s4_to_fp16x2(half2 const &source,
                                                   uint4 *result);

template <>
__inline__ __device__ void dequantize_s4_to_fp16x2<half>(half2 const &source,
                                                         uint4 *result) {
  // uint4 result;
...
dequantize_s4_to_fp16x2<nv_bfloat16>(half2 const &source, uint4 *result) {
  // uint4 result;
  ...
  asm volatile("lop3.b32 %0, %1, %2, %3, %4;\n"
               : "=r"(h[1])
               : "r"(i4s1), "n"(BOTTOM_MASK), "n"(I4s_TO_BF16s_MAGIC_NUM),
                 "n"(immLut));
  // Extract elt_45 - (i4s & 0x000f000f) | 0x43004300
  asm volatile("lop3.b32 %0, %1, %2, %3, %4;\n"
               : "=r"(h[2])
               : "r"(i4s2), "n"(BOTTOM_MASK), "n"(I4s_TO_BF16s_MAGIC_NUM),
                 "n"(immLut));
  // Extract elt_67 - (i4s & 0x000f000f) | 0x43004300
  asm volatile("lop3.b32 %0, %1, %2, %3, %4;\n"
               : "=r"(h[3])
               : "r"(i4s3), "n"(BOTTOM_MASK), "n"(I4s_TO_BF16s_MAGIC_NUM),
                 "n"(immLut));

  // This is the nv_bfloat162 {128, 128} represented as an integer
  static constexpr uint32_t BF16_TOP_MAGIC_NUM = 0x43004300;

  reinterpret_cast<__nv_bfloat162 *>(h)[0] =
      __hsub2(reinterpret_cast<__nv_bfloat162 *>(h)[0],
              reinterpret_cast<const __nv_bfloat162 &>(BF16_TOP_MAGIC_NUM));
  reinterpret_cast<__nv_bfloat162 *>(h)[1] =
      __hsub2(reinterpret_cast<__nv_bfloat162 *>(h)[1],
              reinterpret_cast<const __nv_bfloat162 &>(BF16_TOP_MAGIC_NUM));
  reinterpret_cast<__nv_bfloat162 *>(h)[2] =
      __hsub2(reinterpret_cast<__nv_bfloat162 *>(h)[2],
              reinterpret_cast<const __nv_bfloat162 &>(BF16_TOP_MAGIC_NUM));
  reinterpret_cast<__nv_bfloat162 *>(h)[3] =
      __hsub2(reinterpret_cast<__nv_bfloat162 *>(h)[3],
              reinterpret_cast<const __nv_bfloat162 &>(BF16_TOP_MAGIC_NUM));
}