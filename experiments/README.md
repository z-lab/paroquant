...
We adjust the batch size, learning rate, and number of training samples for LLaMA-3-70B. Please use [`experiments/optimize/4_bit_70b.sh`](./optimize/4bit_70b.sh) instead for LLaMA-3-70B.

The optimized checkpoints will be saved to `./output/<model_name>`. To create a huggingface model with the checkpoints, use [`scripts/pseudo_quant.py`](../scripts/pseudo_quant.py) for pseudo quantization and [`scripts/real_quant.py`](../scripts/real_quant.py) for real quantization.
...
To evaluate downstream tasks, use [`tasks/reasoning.sh`](./tasks/reasoning.sh) for reasoning tasks and [`tasks/non_reasoning.sh`](./tasks/non_reasoning.sh) for non-reasoning tasks.
...
Scripts for ablation studies are in the [`ablations`](./ablations/) directory. Their usage is similar to the ParoQuant optimization script.

## Plots

We provide the scripts to create the plots in the paper in the [`plots`](./plots/) directory.
