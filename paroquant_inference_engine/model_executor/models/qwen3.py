...
from paroquant_inference_engine.model_executor.modules.rotation_linear import RotateLinearInt4
...
class Qwen3MLP(nn.Module):
    def __init__(self, config):
        ...
        self.gate_proj = RotateLinearInt4(self.hidden_size, self.intermediate_size, bias=False, dtype=torch.half)
        self.up_proj = RotateLinearInt4(self.hidden_size, self.intermediate_size, bias=False, dtype=torch.half)
        self.down_proj = RotateLinearInt4(self.intermediate_size, self.hidden_size, bias=False, dtype=torch.half)
...
class Qwen3Attention(nn.Module):
    ...
    def __init__(self, config: Qwen2Config, layer_idx: Optional[int] = None):
        ...
        self.q_proj = RotateLinearInt4(self.hidden_size, self.num_heads * self.head_dim, config.attention_bias, dtype=torch.half)
        self.k_proj = RotateLinearInt4(self.hidden_size, self.num_key_value_heads * self.head_dim, config.attention_bias, dtype=torch.half)
        self.v_proj = RotateLinearInt4(self.hidden_size, self.num_key_value_heads * self.head_dim, config.attention_bias, dtype=torch.half)
        self.o_proj = RotateLinearInt4(self.num_heads * self.head_dim, self.hidden_size, config.attention_bias, dtype=torch.half)
