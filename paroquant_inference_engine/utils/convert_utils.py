...
def _align_shape(pair, angle, grp_size=128, inc_mask=False):
    ...
    while True:
        ...
        while count < grp_size // 2:
            if pair_ptr < pair.size(0) and pair[pair_ptr, 0] - grp * grp_size < grp_size and pair[pair_ptr, 1] - grp * grp_size < grp_size:
...
def transform_from_pt(pt, krot=8, include_qsz=False):
    ...
    channel_scales = channel_scales.unsqueeze(0) if channel_scales.ndim == 1 else channel_scales
    qscales = pt['quantizer.scale']
    qzeros = pt['quantizer.zero_point_float']
    qmin = 0
    qmax = 2 ** n_bit - 1
    round_zero_point = clamp_ste(
        -round_ste(qzeros), qmin, qmax
    )
    if include_qsz:
...
def transform_to_kernel_data(pairs_group: list[torch.Tensor], angles_group: list[torch.Tensor], group_size: int=128):
