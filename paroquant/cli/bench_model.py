...
from paroquant.inference.backends.transformers._decode import decode_one_token, sample_next_token
from paroquant.inference.backends.transformers import load
...
@torch.no_grad()
def benchmark(model, tokenizer, prefill_len, decode_len, past_kv, decode_fn=None):
...
def run_benchmark(hf_path, prefill_len, decode_len, empty_model, max_new_tokens):
    ...
    benchmark(model, tokenizer, 16, 16, past_kv, decode_fn=compiled_decode)
    _, _, decode_tps = benchmark(model, tokenizer, prefill_len, decode_len, past_kv, decode_fn=compiled_decode)
    print(
